...
interface GameHistoryDao {
    ...
    @Query("SELECT * FROM GameHistory WHERE userId = :userId AND mode = :mode ORDER BY sessionStartedAt DESC, gameNum DESC")
    fun observeForUser(
        userId: String,
        mode: GameMode,
    ...
    @Query("SELECT mode, COALESCE(SUM(netChips), 0) AS netChips FROM GameHistory WHERE userId = :userId GROUP BY mode")
